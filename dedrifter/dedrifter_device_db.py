"""
Device database for the ARTIQ Dedrifter crate

This is not intended to be operated with the usual ARTIQ master + controller
manager + GUI etc stack, but is instead a standalone system that runs headless.

This file will be used to configure the system when it's started, but it will
then run headless from then on. See `launch_dedrifter.sh` for details.
"""
