...
from mach_nix.data.providers import ProviderSettings
from mach_nix.exceptions import MachNixError
from mach_nix.generators.overides_generator import OverridesGenerator
from mach_nix.requirements import context
from mach_nix.requirements import filter_reqs_by_eval_marker
...
def do():
    ...
    reqs = filter_reqs_by_eval_marker(
        parse_reqs(requirements), context(py_ver, platform, system)
    )
    reqs = list(reqs)
    try:
...
def handle_resolution_impossible(
    exc: ResolutionImpossible, reqs_str, providers_json, py_ver_str
):
