...
## RISKS / caveats
- **15s ARTIQ build watchdog** (client.py:368-374): a client whose fragment-tree build > 15s is KILLED at submission. Killed EnsureClockPiTimes on 07-12. Heaviest tree = EnsureXODT (self-loads MOT→XODT). This is why blue-canary-FIRST: if blue submits+runs, mechanics are proven; if XODT hits the build watchdog that's a documented lazy-build limitation, NOT an escape-mechanics failure. Record which clients build in time.
