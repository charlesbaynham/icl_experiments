...
class OvenSpectroscopy(EnvExperiment):
    ...
    def pass_Voltage(self, voltage):
        ...
        self.suservo0_ch1.set_iir(profile=0,
        ...
        self.suservo0_ch1.set_dds(profile = int(0),
            offset = -.5,  # 5 V with above PGIA settings
            frequency = self.freq,
            phase = self.phase)
        ...
        self.suservo0_ch1.set(en_out = 0, en_iir = 1, profile = 0)
        ...
        with parallel:
            for j in range(n):
                ...
                for value in voltage:  ## Outputting voltage to the fastino for 461 frequency modulation
                    self.fastino0.set_dac(0, value)
                    delay(50 * us)                    
            
            with sequential:
                for i in range (5000): ## Handling the data of the absorption photodetector
                    self.mutate_dataset("Photodetector_Data", i, self.suservo0.get_adc(0))
                    
                    delay(500 * us)

            with sequential:
                for i in range (5000):  ## Handling the data of the stabilisation photodetector
