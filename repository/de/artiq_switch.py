...
class Urukul_Programmable(EnvExperiment):
    ...
    def build(self): #This code runs on the host device
        ...
        used_uruk = []; used_suservos = []

        for i in range(8):
            string_uruk = "urukul0_ch{it}".format(it = i); string_suservo = "suservo0_ch{it}".format(it = i)            ## We need a few devices, so this activates 4x urukuls and 8x suservo urukuls
            if i < 4: 
                self.setattr_device(string_uruk); self.setattr_device(string_uruk)
