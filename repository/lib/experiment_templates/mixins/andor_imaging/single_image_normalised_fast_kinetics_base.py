    ...
    * :meth:`~update_andor_monitor_hook`
    ...
    def build_fragment(self):
        ...
        self.setattr_param(
            ...
            IntParam,
            "Width of the background ROIs in pixels. Used for both left and right bg ROIs.",
    ...
    def hook_setup_andor(self):
        """
        Setup the Andor camera to use 6x ROIs since we're expecting fast
        kinetics mode with 2 images
        """
...
class SingleImageNormalisedFastKineticsDoubleTrapBase(AndorImagingBase):
    """
    ...
    * :meth:`~update_andor_monitor_hook`
    ...
    def build_fragment(self):
        ...
        self.setattr_param(
            "background_horizontal_width",
            IntParam,
            "Width of the background ROI, in pixels. Used for both left and right bg ROIs.",
            default=constants.ANDOR_SINGLE_FAST_KINETICS_BACKGROUND_ROI_WIDTH,
        )
    ...
    def host_setup(self):
        ...
        # --------------------------------------
        # This seems to be only used for the appet?
        default_rois_ground = [
    ...
    def process_andor_image_hook(self, images: NDArray):
        # TODO This thing here should be looked into, not sure if it's correct
        super().process_andor_image_hook(images)
        ground_bg_corrected = images[0].astype(int)
        excited_bg_corrected = images[1].astype(int)
        self.set_dataset(
