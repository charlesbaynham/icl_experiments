...
class ControlBeamsWithoutCoolingAOM(Fragment):
    ...
    will actually be separated by one RTIO coarse cycle (=8ns normally). This
    is so that only one RTIO lane is consumed, avoiding collisions. If this is
    unacceptable for your application, you will need to manage the lane usage manually.
    ...
    def turn_beams_on(self) -> TInt64:
        ...
        # Open the shutters with the AOMs off
        for i in range(len(self.beam_delays) - 1, -1, -1):
            ...
            # Delay by a coarse cycle to avoid using a new lane
