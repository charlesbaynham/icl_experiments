        ...
        newDefaultShell =
          ...
          (prev: {
            shellHook = ''
              ...
              # The git-hooks.nix shellHook installs hooks that call the raw pre-commit binary,
              # bypassing our PYTHONPATH-unsetting wrapper. Patch them to unset PYTHONPATH so
              # Python 3.13 doesn't accidentally load pre-commit 3.3.2 modules from the shell's
              # python3.10 PYTHONPATH (which causes InvalidConfigError on 'language: unsupported').
              for hook in .git/hooks/pre-commit .git/hooks/pre-push; do
                if [ -f "$hook" ] && grep -q 'File generated by pre-commit' "$hook" 2>/dev/null; then
                  if ! grep -q '^unset PYTHONPATH$' "$hook" 2>/dev/null; then
                    ${pkgs.gnused}/bin/sed -i '/^# end templated$/a unset PYTHONPATH' "$hook"
                  fi
                fi
              done
